	...
	denz.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `OLA @${num.split('@')[0]}👋\nSEJA BEN VINDO(A) AO GRUPO *${mdata.subject}*\n\n ────────────────
┏━━━━━━━━━━━━━━━
┃────「 *_ɪɴᴛʀᴏ_* 」─────
┃━━━━━━━━━━━━━━━
┠⊷️ *NOME* :
...
ᴋᴇᴛɪᴋ ${prefix}daftar ᴜɴᴛᴜᴋ ᴍᴇᴍᴜʟᴀɪ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ʙᴏᴛ`
				...
				teks = `*_「 ᴘʀᴏᴍᴏᴛᴇ ᴅᴇᴛᴇᴄᴛᴇᴅ 」_*\n@${num.split('@')[0]} QUER ADIVINHAR UM ADM?, JA TEM A PERMISSÃO DO DONO DO GRUPO!`
				let buff = await getBuffer(ppimg)
				denz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				...
				teks = `TCHAU @${num.split('@')[0]}-1 INFELIZMENTE\😢😞`
				let buff = await getBuffer(ppimg)
				denz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	denz.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*⏳ WAIT EM PROCESSO...*',
				success: '*SECESSO...*',
				error: {
					stick: '*ERRO AO TRANSFORMA EM FIGURINHA*',
					Iv: '*LINK INVÁLIDO!*'
				},
				only: {
					group: '*ESSE COMANDO SO PODE SER USANDO EM GRUPO*',
					benned: '*SEU NÚMERO ESTA BANIDO*',
					ownerG: '*ESSE COMANDO SO PODE SER USANDO PELO PROPRIETÁRIO DO GRUPO!*',
					ownerB: '*ESSE COMANDO SO PODE SER USANDO PELO DONO DO BOT!* ',
					premium: '*COMANDO SO PARA USUÁRIO PREMIUM!*',
					userB: `OLA*${pushname2}*,VOCE AINDA NÃO ESTA REGISTRADO USE\n*${prefix}daftar* PARA SE REGISTRAR
EXEMPLO *${prefix}daftar nome|idade*`,
					admin: '*ᴍᴀᴀꜰ ᴩᴇʀɪɴᴛᴀʜ ɪɴɪ ʜᴀɴyᴀ ʙɪꜱᴀ ᴅɪ ɢᴜɴᴀᴋᴀɴ ᴏʟᴇʜ ᴀᴅᴍɪɴ ɢʀᴏᴜᴩ!*',
					Badmin: '*ESSE COMANDO SO PODE SER USADO POR ADMIS DO GRUPO!*',
					publikG: `BOT ESTA PRIVADO USE \n*${prefix}infobot* PARA SABER MAIS`
				}
			}

			const botNumber = denz.user.jid
			const ownerNumber = ["5512996378494@s.whatsapp.net","5512996378494@s.whataapp.net"] // owner number ubah aja
